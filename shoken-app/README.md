...
座席表から児童を選び、評価観点別キーワードを選択すると所見文のドラフトを組み立てる、小学校教員向けの通知表所見作成支援ツール。Phase 1は算数のみ・教員1名（自分専用アカウント）での動作を対象とする。文章生成はAPI課金が発生しないルールベース（テンプレート組み立て）方式。
...
- 評価観点別キーワード選択 → テンプレートによる所見文ドラフト生成（文字数指定対応、無料・API不要）
- 同一単元・クラス内の直近の所見文と結びの表現が重複しないようにする簡易ロジック
